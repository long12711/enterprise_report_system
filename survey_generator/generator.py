...
class QuestionnaireGenerator:
    ...
    def __init__(self, indicator_file=None):
        ...
        if indicator_file is None:
            # 使用相对路径，支持灵活配置
            indicator_file = self._get_default_indicator_file()
    ...
    @staticmethod
    def _get_default_indicator_file():
        """获取默认的指标体系文件路径"""
        # 支持多个可能的位置
        possible_paths = [
            r'D:\Claude Code\enterprise_report_system\指标体系.xlsx',
            './指标体系.xlsx',
            '../指标体系.xlsx',
            os.path.join(os.path.dirname(__file__), '..', '指标体系.xlsx'),
        ]
        ...
        # 如果都找不到，返回默认路径（会在load_indicators中报错）
        return r'D:\Claude Code\enterprise_report_system\指标体系.xlsx'
    ...
    def load_indicators(self):
        ...
        try:
            if not os.path.exists(self.indicator_file):
                raise FileNotFoundError(f"指标体系文件不存在: {self.indicator_file}")
